...
def visualize_output(image_path, gt_result_path, pred_result_path, save_into, image_name, is_show=False):
    ...
    # save image
    plt.savefig(os.path.join(save_into, image_name)+'.jpeg')
...
def get_ap(gt_dir, pred_dir, IOUThreshold):
    ...
    metricsPerClass = evaluator.GetPascalVOCMetrics(boundingboxes, IOUThreshold=IOUThreshold)
    ...
    print('Mean Average Precision (mAPs):', round(sum(APs)/len(APs),2),'\n')
    
    return metricsPerClass, APs

def get_individual_score_mod(gt_result_path, pred_result_path):
    '''get individual score for each image.'''
    ...
    for mc in metricsPerClass:
        ...
        # print precision (self)
        precision_self = tp/(tp+fp)
        dscore[c] = {'precision':  precision_self}
        
        # print recall (self)
        recall_self = tp/total_gt
        dscore[c].update({'recall':  recall_self})
        
    cumm_prec = cumm_tp/(cumm_tp+cumm_fp)
    cumm_rec = cumm_tp/cumm_npos
    cumm_f1 = 2 * (cumm_prec*cumm_rec) / (cumm_prec+cumm_rec)
    
    dscore_cumm = {'precision':  cumm_prec, 'recall':  cumm_rec, 'f1': cumm_f1}
    
    return dscore, dscore_cumm